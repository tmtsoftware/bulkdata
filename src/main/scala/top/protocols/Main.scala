...
object Main extends App {
  ...
  args match {
    ...
    case Array("ss", "server", host, Int(port))   => new Server(host, port, new ServerSentProtocol(Image.stream)).run()
    case Array("sr", "server", host, Int(port))   => new Server(host, port, ServerReceivedProtocol).run()

    case Array("bidi", "client", host, Int(port)) => new Client(host, port, new ClientProtocol(Image.stream)).run()
    case Array("ss", "client", host, Int(port))   => new Client(host, port, new ClientProtocol(Image.single)).run()
    case Array("sr", "client", host, Int(port))   => new Client(host, port, new ClientProtocol(Image.stream.take(10))).run()
