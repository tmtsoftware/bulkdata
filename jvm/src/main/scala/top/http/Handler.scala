...
class Handler(implicit mat: Materializer, executor: ExecutionContext) {
  ...
  val requestHandler: HttpRequest => Future[HttpResponse] = {
    ...
    case HttpRequest(HttpMethods.POST, Uri.Path("/images"), _, entity, _) =>
      val images = entity.dataBytes.map(ImageConversions.fromByteString).log("Server-Received")
    ...
    case HttpRequest(HttpMethods.POST, Uri.Path("/images/bidi"), _, entity, _) =>
      val images = entity.dataBytes.map(ImageConversions.fromByteString).log("Server-Received").map(_.updated)
      val chunked = HttpEntity.Chunked.fromData(ContentTypes.NoContentType, images.map(ImageConversions.toByteString))
      Future.successful(HttpResponse(entity = chunked))
